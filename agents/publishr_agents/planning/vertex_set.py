...
async def _run_team(
    ...
    """1チームの縦串（調査トリオ→plan_owner）を実行し PlanProposal を返す。"""
    init_state: dict[str, Any] = {
        ...
        K.REJECTION_FEEDBACK: rejection_feedback,
    }
    st = await _run(build_team_pipeline(), _APP_TEAM, init_state)
    return _norm_plan(st.get(K.PLAN_DRAFT))
