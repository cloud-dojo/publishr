...
from publishr_schema import ObservationBundle, ReaderProfile3Layer, User
...
def analyze_reader(
    ...
    *,
    ...
    if mode == "mock":
        return analyze_reader_deterministic(observation, user=user, prev_profile=prev_profile)
    if mode == "vertex":
        from .vertex_agent import analyze_reader_vertex

        return analyze_reader_vertex(observation, user=user, prev_profile=prev_profile)
