...
from publishr_agents.planning.deterministic import (
    derive_theme,
    derive_theme_set,
...
# ══════════════════════════════════════════════════════════════════════════
# v3 4テーマ1-1-1-1 セット企画（予約制廃止改定 2026-06-23・オフライン土台）
# ══════════════════════════════════════════════════════════════════════════
def test_theme_set_has_four_assignments_with_teams():
    tas = derive_theme_set(_profile())
    ThemeAssignmentSet.model_validate(tas.model_dump(by_alias=True))  # スキーマ検証
    assert [a.team_id for a in tas.assignments] == ["A", "B", "C", "D"]
    # activeWorkThemes 先頭を主軸に据える（診断の的中を棚から落とさない）
    assert tas.assignments[0].theme.role == "主軸"
    assert tas.assignments[0].theme.name == _profile().current_work.active_work_themes[0]
    assert tas.editorial_intent and tas.editorial_intent.shelf_concept


def test_planning_set_produces_four_plans_1_1_1_1():
    result = run_planning_set_deterministic(_profile(), threshold=70)
    ps = PlanSet.model_validate(result["planSet"])
    assert ps.allocation == "1-1-1-1"
    assert len(ps.plans) == 4
    assert len({p.proposal_id for p in ps.plans}) == 4  # 4冊が別企画


def test_planning_set_research_trio_per_theme():
    result = run_planning_set_deterministic(_profile())
    assert len(result["research"]) == 4
    for trio in result["research"].values():
        assert trio["subTrend"] and trio["subMarket"] and trio["subThemeInsight"]
        assert trio["subMarket"]["marketGap"]


def test_planning_set_axis_spread_is_diverse():
    """棚の多様性＝感情トーン4軸が分散（axisSpread の素地）。"""
    result = run_planning_set_deterministic(_profile())
    plans = PlanSet.model_validate(result["planSet"]).plans
    tones = {p.emotional_tone for p in plans}
    assert len(tones) == 4  # トーンが4冊で散る
    assert result["planSetVerdict"]["portfolio"]["axisSpread"] >= 3


def test_planning_set_reject_then_approve_trace():
    """セットゲートの差し戻し→再提出→承認＋reject_log 証跡（基準1の核・セット版）。"""
    result = run_planning_set_deterministic(_profile(), threshold=70)
    history = result["verdictHistory"]
    assert history[0]["decision"] == "revise" and history[0]["score"] < 70
    assert history[-1]["decision"] == "approve" and history[-1]["score"] >= 70
    assert result["rounds"] == 2
    assert result["rejectLog"], "却下証跡（reject_log）が残る"
    assert result["rejectLog"][0]["belowFloor"], "差し戻し時に足切りの冊が記録される"


def test_planning_set_verdict_validates_and_selective_revise():
    result = run_planning_set_deterministic(_profile(), threshold=70)
    PlanSetVerdict.model_validate(result["planSetVerdict"])  # 承認版が検証できる
    # 最終は4冊承認・portfolio allocation_ok
    pv = result["planSetVerdict"]
    assert pv["decision"] == "approve"
    assert pv["portfolio"]["allocationOk"] is True
    assert len(pv["perPlan"]) == 4


def test_planning_set_deterministic_is_stable():
    a = run_planning_set_deterministic(_profile(), threshold=70)
    b = run_planning_set_deterministic(_profile(), threshold=70)
    assert a == b
