...
3. **アプリを開くと棚に新刊が入荷**（各冊に装丁付きの表紙＋入荷理由付き）— **届いた瞬間に全文が読める**（配本runで作り切り済み）
...
graph TD
    ...
    subgraph MODEB[モードB: 本文執筆（企画後に1冊=1ジョブで自動・最高3R）]
        EDITOR -->|1冊ずつ自動enqueue| PUBSUB[Pub/Sub]
        PUBSUB --> WRITER[本文編集ループ<br/>編集長 ⇄ 著者 Pro]
        WRITER --> GCS[Cloud Storage<br/>本文MD]
    end
...
- 🎨 **表紙（CSS 装丁）** — 各冊に決定的な CSS 装丁（10種の暗色グラデ）で表紙を付与（オフライン・ゼロコスト）。※AI 画像生成（Vertex Imagen）の表紙は今回スコープ外で park・将来実装
...
| `ENABLE_IMAGEN` | 表紙を Imagen 生成する ※画像生成 park により現行 no-op（将来実装） | `false` | `false` |
...
| `PUBLISHR_BODY_BUCKET` / `PUBLISHR_COVER_BUCKET` | 本文 MD / 表紙 PNG の非公開バケット（COVER_BUCKET は画像生成 park により現行未使用） | `publishr-contents-498123` | 同左 |
...
gcloud run deploy publishr-api \
  ...
  --update-env-vars=PUBLISHR_BODY_STORE=gcs,PUBLISHR_BODY_BUCKET=publishr-contents-498123 \
...
Publishr/
├── README.md                     ← 本ファイル
├── agents/publishr_agents/       ← ADK マルチエージェント（モードA/B・観測/企画/著者/本文）
├── apps/
