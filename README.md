...
2. Cloud Scheduler が自動起動（水・土 朝6時）→ Drive/Calendar/Tasks を観測し企画3階層会議
3. **翌朝、アプリを開くと棚に新刊が入荷**（各冊に AI 生成（Imagen）の表紙＋入荷理由付き）
4. 本を開くと**全文がもう書き上がっている**（企画と同時に本文まで自動執筆済み＝手動「予約」は不要）→ 読む
...
- 📖 **入荷した瞬間に全文が読める** — 企画と同時に本文まで自動執筆。1冊=1ジョブ（Pub/Sub）で約100pを編集長⇄著者ループで仕上げ published に。手動「予約」は不要。本文は非公開 Cloud Storage に退避し、サーバ側 read で配信
- 🎨 **AI 生成の表紙** — Vertex Imagen で1冊ずつ表紙を生成（非公開 GCS＋サーバ側配信）。生成失敗時は CSS 装丁にフォールバック
- 📚 **書店メタファーUI** — 「入荷・読む・書庫」の自然な動線。入荷理由を付記してAIの判断を可視化。書庫は published を積み上げ（消えない）
...
| W4 | 6/29〜7/5 | フロントUI・Eval CI・マルチユーザー・企画自動執筆・表紙Imagen | ✅ ほぼ完了 |
