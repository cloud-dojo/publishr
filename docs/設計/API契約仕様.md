...
> **ステータス**: 🟡 ドラフト（2026-06-02）。§6の未確定を友人MTGで詰める。
...
#### 選択肢（確定・正本＝`frontend/src/data/profileOptions.ts`）
...
ユーザーが棚の1冊（著者版）を選び「予約」する。Firestoreの状態更新とPub/Sub発行を**1エンドポイントで原子的に**行う。
...
4. `status != "draft"` なら `409`（既に予約/執筆/公開済み）。
5. **予約上限チェック（★同時最大5冊・I-16解決）**: 当該ユーザーの `status in (reserved, writing)` の冊数を数え、**5冊以上なら `409`（`reservation_limit_exceeded`）**。単位＝**同時**（reserved+writing の合計）。本文Pro生成のコスト天井（MVPスコープ §5-2）。
6. `status: draft → reserved` に更新。
7. Pub/Sub トピック `book-writing` に `{ bookId }` を発行（→ モードB 本文編集ループ：編集長⇄著者 最高3R）。
8. 成功レスポンス。
...
- 処理：`state`検証 → 認可コードをトークンに交換 → **refresh_token を Secret Manager / Firestore（暗号化）に保存**（生トークンを平文でFirestoreに置かない）→ `users/{uid}.connectedSources.{drive,calendar,tasks}.enabled=true` を更新。
- Response：フロントの設定完了画面へリダイレクト。

> **保存先の最終決定は友人MTG**（Secret Manager推奨。Firestoreに置くなら暗号化必須）。MVPは個人＋テストモードのため当人のトークン1組で足りる。
...
### 処理
1. トークン検証 → `uid`。（MVPは特定デモアカウントのみ許可してもよい）
...
> 本書に関する未確定論点は **`未決論点台帳.md` に集約**（関連: G1-3 予約の発火方式／G1-5 OAuthトークン保存先／G1-6 手動トリガー認可範囲／G1-7 APIベースパス・CORS／G1-4 ownerUid／I-5 initialProfile変更可否／I-7 favoriteAuthors上限／F-1 混入比率）。
