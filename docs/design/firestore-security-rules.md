...
> **ステータス**: ✅ MTG 2026-06-05で関連論点を確定（ownerUid方式・読書ログ feedback集約 I-9・観測束 observations サブコレクション I-19）。本書はその確定を反映済み。⚠️ 2026-06-06時点のコード確認では `firestore.rules` 実ファイルは未作成のため、`NEXT_PUBLIC_DATA_SOURCE=firestore` や本番App Hosting接続より前に、本文ルールを実ファイル化・デプロイ・emulator testで検証する。
...
- **実装ゲート**: `firestore.rules` が存在しない、またはデプロイ済みルールが本書と一致しない状態で、Firestore直読み/直書きの本接続を有効化しない。特に `users/{uid}/observations/{date}` はDrive/Calendar/Tasks由来の機微データを含むため、本人read・クライアントwrite禁止を必須にする。
