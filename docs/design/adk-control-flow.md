...
> **担当**: 友人（エージェント基盤）。鉄田がドラフトを用意し、友人MTG（2026-06-05）で「一旦ADKでいく」方針を握り済み（G1-1）。
> **ステータス**: 🟢 制御フロー論理設計は確定。**方針＝一旦ADKでいくで合意（G1-1・MTG 2026-06-05。論点は残るがW1 Hello Worldで実証しながら進める。W1末6/14までに動かなければSTEP2のみLangGraphへ逃げるNGラインも合意）**。⚠️ ADKの具体API名（escalate/max_iterations 等のクラス・メソッド）は§9のとおり**W1 Hello Worldで要実証**。
