...
ユーザーが棚の1冊（著者版）を選び「予約」する。Firestoreの状態更新とPub/Sub発行を**1エンドポイントで原子的に**行う。**予約の原子性は Firestore transaction で担保（I-20＝MTG 2026-06-05確定）**＝「reserved+writing の冊数を読む→ <5 を確認→ `draft→reserved` 遷移」を1単位で実行し、レースで6冊目が通るのを防ぐ。**【2026-06-12 実装】** `RepositoryProtocol.reserve_book_atomic(book_id, owner_uid, max_concurrent)`＝mock は `threading.Lock`、firestore は `@firestore.transactional`（txn内で owner の active 件数を数える）。owner 別 cap（検証済み uid を endpoint から渡す）。複合インデックス `ownerUid+status` を `firestore.indexes.json` に追加済み。
...
> **【ハードニング・C4.9/G1-21】** ①**state リプレイ**＝**nonce 単回化を実装済（2026-06-12・`oauth_service.NonceStore`・`build_auth_url` がランダム nonce 生成→callback で `consume`・replay は 403）**。残＝**開始ブラウザとの束縛**（nonce クッキー or PKCE・要サーバ側保持）。②**レート制限**＝**`/start`・`/connect/drive-folders` は per-uid 実装済（2026-06-12・`services/rate_limit.RateLimiter`・429）**。残＝`/callback`（署名検証で短絡＝低優先）。③**マルチインスタンス**＝nonce/レート制限は現状インメモリ＝1プロセス内。複数インスタンスでは共有ストア（Firestore/Redis）が要る（HMAC署名＋TTLはインスタンス跨ぎで有効＝主防御は維持）。④**drive スコープ**＝実装既定は `drive.readonly`（フォルダ列挙に必要・restricted）。`drive.file`（最小権限）はフォルダ列挙と非互換のため、デモは `PUBLISHR_GOOGLE_SCOPES` で drive を外す運用（calendar+tasks）。
