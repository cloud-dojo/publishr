...
| ID | タスク | タスク詳細（何をやる？） | 担当 | 予定週 | 依存 | DoD | 状態 |
|---|---|---|---|---|---|---|---|
| C5.1 | W1 各プロンプト実テスト→調整 | 実際にAIへ指示文を流し、想定どおり出るか・悪い例を弾くかを確認して調整 | 鉄田・一瀬 | W1–W2（6/8–21） | C1.0.1 | Langfuseで出力確認 (旧WP5.3) | 🟡**ハーネス整備（2026-06-14・一瀬）**＝①単発ランナー `scripts/run_prompt.py`（任意1ロールの最終 system＋user を組み立て mock表示/実Vertex 実行・`--role/--input/--from-eval/--backend/--list`・subs はセクション切出し・grounded/structured 自動判定）②`eval_harness.check_prompt_files_loadable`＝**全11プロンプト**の system 抽出＋良い例(✅)健全性を決定的に回帰（few-shot 黙殺退行を CI が捕捉）③入力fixture `eval/prompt_inputs/reader_analyst.json`（実観測由来）④gated `@pytest.mark.vertex` スモーク。MiniLoopで research_subs/plan_owner/plan_leader は実Vertex確認済（C1.0.1）。**残＝実Vertex（us-central1・課金）で全11本を流し、想定出力/悪い例拒否を確認→プロンプト微調整（鉄田と・Pro quota 配慮）** |
