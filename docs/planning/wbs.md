...
| **C4前** | Cloud Run外部公開＋`PUBLISHR_LLM=vertex`＋`DATA_SOURCE=firestore` の前に: Firebase IDトークン検証・Firestore rulesデプロイ（**C3.1**）・OAuth `state` 検証・手動トリガー許可uid・レート制限 | C4.9 本番接続 | ✅**実装済（2026-06-13棚卸し）**＝IDトークン検証(`api.py:_verify_uid`)／rulesデプロイ(C3.1✅)／OAuth `state` HMAC検証＋nonce単回化(`oauth_service`)／許可uid・連打ロック(`trigger_guard`)／rate limit(`rate_limit.py`) を全て実装＆main入り。実課金デプロイ時は `PUBLISHR_REQUIRE_RESERVE_AUTH=true`＋`allowed_trigger_uids` で発火 |
...
| B2.2 | ローカル環境統一（Python3.12/ADK SDK/Node22） | 2人のPCで同じバージョンのツールを使うよう揃え、動作の食い違いを防ぐ | 鉄田・一瀬 | W1（6/8–14） | B2.1 | バージョン固定 (旧WP0.6) | 🟡**実態は統一済（棚卸し2026-06-13）**＝**Python 3.12**（`pyproject.toml` `requires-python>=3.12`・`uv sync` で自動選択・CIも 3.12）＋**Node 22**（CI `Setup Node 22`）＋ADK は uv lock 固定。旧表記「3.11」は誤り。devモードも整備済（`dev:mock`／`dev:emulator`＋Firebaseエミュレータ・要Java・`docs/infra/local-sandbox.md`）。**任意残＝`.python-version` 明示ピン（無くても uv が解決）** |

## B3. CI/CD・ホスティング（Actions/Cloud Build/App Hosting）← 鉄田がGitHub連携を実施（組織移管・オーナー権限完了）
| ID | タスク | タスク詳細（何をやる？） | 担当 | 予定週 | 依存 | DoD | 状態 |
|---|---|---|---|---|---|---|---|
| B3.1 | CI/CD空パイプライン疎通 | コードをpushしたら自動でテスト→公開が動く「配線」を、まず中身カラ（"Hello"表示だけ）で通して土台を作る | 一瀬 | W1（6/8–14） | B2.1 | push→Actions→Cloud Run "Hello"（C1.0と兼用） (旧WP0.5) | ✅**完了（実CIに発展・棚卸し2026-06-13）**＝`.github/workflows/ci.yml`＝push→verify(pytest+web lint/typecheck)→eval→eval-gate→WIF keyless で Cloud Run 自動デプロイ。実機検証済（PR#15 CI／PR#17 CD・rev実機200）。「空疎通」段階は実パイプラインに包含。docs-only push の deploy スキップ gate も追加（PR#29） |
...
| C3.2 | 複合インデックス列挙＋firestore.indexes.json | 一覧画面の検索が速く正しく動くよう、DBに索引を登録する（無いと実行時エラーになる） | **鉄田**・一瀬 | W2–W3（6/15–28） | C4.2,C4.7 | 実行時エラー回避（I-15） (旧WP3.2) | ✅**完了（2026-06-13）**＝I-20予約transactionの `books: ownerUid+status` 複合indexを `firestore.indexes.json` に宣言＋`firebase deploy --only firestore:indexes` で**本番反映済**（既存複合index 0件＝純追加・非破壊）。`list_books` の多重equalityは単一fieldインデックスで足り複合不要。新クエリ追加時はここへ追記 |
| C3.3 | GCS本文保護（署名付きURL or IAM） | 本文ファイルを他人が読めないよう、アクセス制限をかける | **鉄田**／一瀬(バックエンド) | W3（6/22–28） | C2.3 | 他者本文を読めない（I-10） (旧WP3.3) | 🟡**バックエンド code-complete（2026-06-14）**＝本文オフロードのシームを実装。`services/body_store.py`（`GcsBodyStore` put/get/signed_url・遅延import・`google-cloud-storage` は `[gcs]` extra）＋`reservation_service._persist_published`（published 永続化を集約・`PUBLISHR_BODY_STORE=gcs` 時のみ本文を **非公開** バケットへ退避し doc には `bodyUrl` だけ残す・既定 inline は従来挙動＝**mock不変**）＋`GET /api/books/{id}/body`（所有者チェック→サーバ側 read で本文返却＝GCSオブジェクトを晒さない）＋フロント `firestore-provider` の hydrate（`bodyUrl` 有り＆`body` 空のみ・リーダー無改修）。test 11件（`test_body_store`/`test_body_offload`・FakeStore で seam 検証・$0）。**【2026-06-14 実GCS live 検証＋本番配線】**＝`GcsBodyStore.put→get` を実バケット `publishr-contents-498123`（ASIA-NORTHEAST1）に実行成功（`test_body_store_gcs.py`・`@pytest.mark.google`・runner SA は project レベル `roles/storage.objectAdmin` 保有）。本番 Dockerfile に `[gcs,secret-manager]` extra 同梱＋`ci.yml` deploy に `PUBLISHR_BODY_STORE=gcs,PUBLISHR_BODY_BUCKET=publishr-contents-498123` 注入（merge で Cloud Run 自動反映＝以降 published 本文は GCS 退避）。**残＝① 署名URL直リンク（runner SA に `roles/iam.serviceAccountTokenCreator`＝SignBlob 付与・現状 reader は server-side read なので不要）② 約100p 化（C2.3） ③ 表紙(cover)の GCS 退避（`imagen.py`・別follow-up）** |
...
| ID | タスク | タスク詳細（何をやる？） | 担当 | 予定週 | 依存 | DoD | 状態 |
|---|---|---|---|---|---|---|---|
| C5.1 | W1 各プロンプト実テスト→調整 | 実際にAIへ指示文を流し、想定どおり出るか・悪い例を弾くかを確認して調整 | 鉄田・一瀬 | W1–W2（6/8–21） | C1.0.1 | Langfuseで出力確認 (旧WP5.3) | 🟡**ハーネス整備（2026-06-14・一瀬）**＝①単発ランナー `scripts/run_prompt.py`（任意1ロールの最終 system＋user を組み立て mock表示/実Vertex 実行・`--role/--input/--from-eval/--backend/--list`・subs はセクション切出し・grounded/structured 自動判定）②`eval_harness.check_prompt_files_loadable`＝**全11プロンプト**の system 抽出＋良い例(✅)健全性を決定的に回帰（few-shot 黙殺退行を CI が捕捉）③入力fixture `eval/prompt_inputs/reader_analyst.json`（実観測由来）④gated `@pytest.mark.vertex` スモーク。MiniLoopで research_subs/plan_owner/plan_leader は実Vertex確認済（C1.0.1）。**【2026-06-14 全11本 実Vertex 実走完了（us-central1・課金）＝spec準拠・プロンプト調整不要】**＝reader_analyst（3層・表層→診断→次に効く力・本人未言及・evidence source 紐付け＝良い例水準）／subs（grounding 実書名・著者・実 source URL）／owner+leader（読者局面直撃企画を採用・eval_01=94/approve）／persona+author+editor（e2e で「役員が決裁するのは、ストーリーだ。」＝具体・エッジ・「入門」型でない）／cover（variant 付与）／modeb_author+modeb_editor（設計思考・ペルソナ声・節末アクション→編集長 approve）／eval_judge（eval_01=90∈[70,100]・eval_05=20∈[0,40] 関連性を正しく弁別）。**唯一の要調整＝borderline ケースの eval帯 vs 実Pro 決定性（eval_b1/eval_b2）→ C5.5 帯整合（鉄田）**。明確ケースは完全整合。 |
