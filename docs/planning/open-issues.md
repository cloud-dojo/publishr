...
| G1-16 | **LLMコスト概算と上限ガード方針** | 🟡ドラフト済／**予約制廃止改定で前提更新** | `../design/cost-estimate.md` 作成済（ハイブリッド＋~~予約上限5冊~~＋編集ループ）。**【予約制廃止改定 2026-06-23】コスト天井は「予約上限5冊」でなく「週3回×4冊＝週12冊の固定配本＋全冊本文生成」を前提に概算を見直す（全冊フル生成でコストは上振れ方向＝要再試算）**。dev/prodガードは `.env.example` に追加済（BODY_PAGE_COUNT/ENABLE_IMAGEN/BATCH_BOOKS_OVERRIDE）。残＝W1実測で実値上書き |
...
| I-17 | セレンディピティの冊数 | ✅**2026-06-18 MTGで確定**／**保持は予約制廃止改定で更新** | **日曜1セット（4冊）に決定**（旧暫定5冊→新4冊）。週3回×4冊/回=週12冊のうち、日曜の1回分（4冊）をセレンディピティセットとして提供。通常入荷（本命）は停止。週計: 本命8冊（2回×4冊）＋セレンディピティ4冊（1回×4冊）=12冊。**【予約制廃止改定 2026-06-23】保持＝~~4週保持=最大48冊~~→入荷から1週間で棚落ち（`ARRIVAL_WINDOW_DAYS=7`）・書庫移動分のみ永久保存（I-29/AGENT §1）**【MTG 2026-06-18・3.2】 |
...
| I-29 | **入荷保持期間・棚落ち＋書庫の仕組み** | 🔴未実装／**方針は予約制廃止改定で再確定（28日化は撤回）** | ~~MTG 2026-06-18: 7日→過去4週間（28日）・最大48冊~~ → **【予約制廃止改定 2026-06-23】保持＝入荷から1週間で棚落ち（`ARRIVAL_WINDOW_DAYS=7` を維持＝28日化は撤回）。書庫へ移した本（`books/{bookId}.archivedAt` セット）だけ永久保存し、未保存は7日経過で入荷一覧から消滅**。実装＝フロント `apps/web/src/lib/arrival.ts` は `ARRIVAL_WINDOW_DAYS=7` のまま、入荷一覧フィルタを「`arrivedAt+7日 > now` または `archivedAt` あり」に。物理削除バッチは任意【AGENT §1/§11・arrival.ts】 |
| I-30 | **動的フィルタリング（書庫移動後の入荷一覧非表示）** | 🔴未実装（2026-06-18 MTG決定） | **MTG 2026-06-18 で確定**。ユーザーがアイテムを「私の書庫」へ移動（reserve→writing→published 完了）した際、**入荷一覧（arrivals）からは非表示**とし、書庫ページのみで表示する。目的: 一覧のノイズを排除し未読・未予約アイテムへの視認性を確保。実装: `books.shelf` が `library` になった本を書店トップの arrivals 一覧から除外するフィルタ（BFF or フロント）を追加【MTG 2026-06-18・3.2・books.tsx・arrival.ts】 |
| I-31 | **デモ用即時入荷トリガーボタン** | 🔴未実装（2026-06-18 MTG決定） | **MTG 2026-06-18 で確定**。バックエンドの入荷処理（`POST /api/trigger/planning`）を即時実行するためのデモ用UIボタンを実装し、動作確認のリードタイムを短縮する。現在は許可uidリスト制限済みエンドポイントが存在するが、フロント側のトリガーボタンが未設置。実装候補: アカウント画面またはデバッグ専用ページに配置（デモ垢のみ表示）【MTG 2026-06-18・3.3・G1-6】 |
...
| I-33 | **著者お気に入り（favoriteAuthors）状態保持バグ（優先度: 高）** | 🔴バグ（2026-06-18 MTG確認） | **MTG 2026-06-18 で確認・修正タスクへ割当**。`favoriteAuthors` 設定が次セッションや状態遷移時に正しく引き継がれない。Firestore の `users/{uid}.favoriteAuthors` への永続化タイミング（`arrayUnion` 呼び出し後の再読込・キャッシュ無効化）またはプロバイダ側の hydrate 処理に不具合がある可能性。修正優先度: 高（次回企画の15%混入シグナルに直結）【MTG 2026-06-18・WBS C4.6・firestore-security-rules】 |
...
- ✅ **【予約制廃止改定・2026-06-23】配本モデルを大改定**：①**配本＝4テーマ・各1冊（1-1-1-1・週3回×4冊＝週12冊）**②**予約制（モードA/B区別・モードB後追い執筆・`POST /reserve`・同時上限5冊・Pub/Sub執筆発火）を廃止**＝各配本runで**全4冊を予約を待たず本文まで作り切って published**③**棚＝入荷から1週間で棚落ち（消滅・`ARRIVAL_WINDOW_DAYS=7`）、書庫へ移した本だけ永久保存**。docs反映済＝AGENT §1/§7/§9/§11・wbs C1/C2・open-issues I-16(廃止)/I-17/G1-3/G1-16。**⚠️要交通整理＝既実装の予約フロー（C2.1-2.3 mock・C4.4 予約UI・M3達成分）と旧3テーマ配線（未コミットの state_keys/provider/__init__・agent_io の PlanSet系）は本改定で要見直し**【AGENT §1・wbs C1/C2】
...
- ~~✅ 週3回・曜日別トリガー（土/水=本命・日=セレンディピティ）／読者分析は土朝の週1回【2026-06-03】~~ → **【2026-06-18 MTG改定】** 週3回×4冊/回=週12冊（本命2回×4冊=8冊＋日曜セレンディピティ1回×4冊=4冊）。 ~~保持4週間・最大48冊~~ → **【予約制廃止改定 2026-06-23】保持＝入荷から1週間で棚落ち（`ARRIVAL_WINDOW_DAYS=7`）・書庫移動分のみ永久保存**（I-17/I-29更新）
...
- ✅ 本文ボリューム＝約100ページ【確定済】
- ~~✅ 週15冊の表紙生成＝MVP/デモは全冊先行生成【2026-06-03】~~ → **【2026-06-18 MTG改定】** 週12冊（4冊×3回: 本命2回+セレンディピティ1回）=週12冊。 ~~4週保持48冊~~ → **【予約制廃止改定 2026-06-23】1週間棚落ち＋書庫永久（I-29）。表紙含め全冊先行生成方針は維持＝予約を待たず本文まで作り切る**
