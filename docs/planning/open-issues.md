...
| I-40 | **plans onSnapshot が全件queryで permission-denied** | 🟡未修正（2026-06-24発覚・実害軽微） | `firestore-provider.ts:102` が `onSnapshot(collection("plans"))` を **ownerUid filter 無し**で購読→rules `allow read: ownerUid==uid` がコレクションqueryを拒否→`permission-denied`（コンソールに恒常エラー）。実害＝クライアントで plans が読めず入荷理由が `deliveryReason` にフォールバック（表示自体は出る）。修正＝`where("ownerUid","==",uid)` を付与（books listener と同様）【firestore-provider.ts:102】 |
