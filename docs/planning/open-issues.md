...
| I-41 | **BFF `ALLOWED_TRIGGER_UIDS` 空＝バックエンド・トリガーゲートが開放** | 🟡要ハードニング（2026-06-24） | 方針A多層防御のバックエンド層が未設定。Cloud Run env に `ALLOWED_TRIGGER_UIDS` が無く `TriggerGuard` は「空=全許可」（trigger_guard.py:26）。フロントはボタンを佐倉のみ表示(`NEXT_PUBLIC_TRIGGER_UIDS`・本QAで apphosting に投入済)だが、**署名済みなら誰でも `POST /api/trigger/planning` を実Vertex企画(課金)できる**（rate limit のみ）。一瀬対応＝Cloud Run に `ALLOWED_TRIGGER_UIDS=5JLLGOc3rpXiGN9KXmsISBNAKty2` 設定【config.py:87・trigger_guard.py・prod-live-followups #7】 |
