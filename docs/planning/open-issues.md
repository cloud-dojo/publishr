...
> ✅**2026-06-06〜2026-06-08で全項目実証済み**（C1.0.1 MiniLoop＋C1.1-C1.7 モードA全STEP・wbs.md参照）。以下はW1着手前に立てた検証観点の記録として残す（記憶での断定をせず、公式ドキュメント/SDKに対して実証する方針・スキル `google-agents-cli-adk-code` 参照）。【出典：ADK §9】
...
| # | 論点 | 状態 | メモ／出典 |
|---|---|---|---|
| I-1 | スコア閾値の最終値の運用調整 | ✅**70で確定（2026-07-01 実データ検証済）** | 実データで調整。**2026-06-23：実Pro品質バッチで leader/eval_judge が96〜97の満点近辺へドリフトを確認→両者に同一文言のレンジ規律を追加（各観点25点=非の打ち所なし限定／総合90+は卓越例外／通常良作72〜88）。leader 実測89へ降下・high帯[70,100]維持で eval-safe・make eval 緑。**【2026-07-01 C5.4/5.5実測で運用調整完了】** 実judge再現性測定（`eval_reproducibility --backend vertex` n=5）で全8ケースband内5/5・CV0.004-0.137、実データ閾値スイープ（`eval_gate --backend vertex`実測値ベース）で70は本命4/4・低関連0/2を維持し妥当性を確認。**閾値70は据え置き確定**。境界ケース(eval_b1/b2)がmock設計と実judge挙動でband乖離あり＝新規I-42参照**【ADK §9-10・AGENT §10-7・I-37・I-42】 |
...
| I-25 | **serendipity 採点の較正（読み替え①の安定性・境界帯）** | ✅**実LLM実測で解消（2026-07-01）** | STEP2手動検証（2026-06-12）で、leader/eval_judge の①relevance を serendipity時に「嗜好・許容度との整合」へ読み替える方針を確定（中レンジ30-60を廃止しモノサシ統一・`1bfc2ac`／一瀬合意済み）。**ただし AI Studio 実測で同一企画群の①が大きく振れた**（条項なし8→あり23・形式不備R1=8→修正R2=23）。few-shot錨（leaderにserendipity採点例1件追加）で緩和したが、70点近傍の serendipity 企画を安定して裁けるかは未検証だった。**【2026-07-01 実judge実測（鉄田）】** `eval_reproducibility --backend vertex` でeval_07/eval_08（serendipity）を5回採点＝mean88.0/86.8・σ0.63/1.47・CV0.007/0.017・band内5/5・一致100%＝**全10ケース中最も安定**。70点近傍での不安定化は観測されず、①読み替え規律は実judgeで高精度に機能。`eval_gate --backend vertex`単発でも89/88とband内で一貫。**懸念は解消・追加調整不要**。関連 G1-8/I-1/I-18/I-21【packages/prompts/{step2_plan_leader,eval_judge}.md・2026-06-12・2026-07-01】 |
...
| I-29 | **入荷保持期間・棚落ち＋書庫の仕組み** | ✅**本番実機検証済（2026-06-24・佐倉ログイン）**：30日以内の本が新しい順表示・archivedAt 済は入荷から除外を確認（I-30修正後） | ~~MTG 2026-06-18: 7日→過去4週間（28日）・最大48冊~~ → **【2026-06-24確認】保持＝入荷から30日で棚落ち（`ARRIVAL_WINDOW_DAYS=30`）。書庫へ移した本（`books/{bookId}.archivedAt` セット）だけ永久保存し、未保存は30日経過で入荷一覧から消滅**。実装＝フロント `apps/web/src/lib/arrival.ts` は `ARRIVAL_WINDOW_DAYS=30`、入荷一覧フィルタは「`createdAt+30日 > now` かつ `archivedAt` なし」、書庫ページは `archivedAt` ありも表示。物理削除バッチは任意【AGENT §1/§11・arrival.ts】 |
| I-30 | **動的フィルタリング（書庫移動後の入荷一覧非表示）** | ✅**本番実機検証済（2026-06-24）＝2段の修正で解消** | 書庫保存は `books/{bookId}.archivedAt` をセットし、書店トップは `isVisibleArrival()` で「30日以内かつ未保存」の本だけ表示する。書庫ページは `archivedAt` あり、または `shelf=library` の本を表示する。`mock`/`firestore`/`bff` provider に `saveToLibrary` を追加し、Firestore rules は本人の `archivedAt` 更新を許可【MTG 2026-06-18・3.2・arrival.ts・provider.ts・firestore.rules】。**【2026-06-24 本番QAで2件の未達を発見・修正】(A) 本番Firestore rules が archivedAt 許可(PR#83)を未デプロイ→`archivedAt`書込が permission-denied→`firebase deploy --only firestore:rules` で解消。 (B) 実装が記述と乖離＝home/library が `isVisibleArrival/isArchivedBook` を使わず生 `shelf` で絞っていた（saveToLibrary は archivedAt のみ更新するため書庫移動が無反映）→commit `71e16a9` で home=isVisibleArrival・library=isArchivedBook&&!dropped に修正・デプロイ→実機で「書庫移動→入荷から消え書庫に出現」を確認** |
| I-31 | **デモ用即時入荷トリガーボタン** | 🟡**本番実機で一部検証（2026-06-24）**：表示ゲート・受理OK／フロントenv投入で解消／ただし worker は I-39 でクラッシュ・I-38 ストーム | アカウント画面に `mock` では非表示のデモ用「今すぐ入荷を実行」ボタンを追加。`bff` は `POST /api/trigger/planning` 実行後に本一覧を再読込、`firestore` は既存 API 呼び出し＋Firestore購読で反映。既存のmock書店・初回体験（`runFirstRun`）には触れない実装にした。**【2026-06-24 本番QA】(1) 表示ゲート＝apphosting に `NEXT_PUBLIC_TRIGGER_UIDS=5JLL…` 未投入でボタンが佐倉に出ない不具合→commit 投入＋デプロイで佐倉のみ表示を確認。(2) 本命/セレンディピティとも BFF 受理 `trigger ok queued=True llm=vertex`・実Google観測 live を確認。(3) ただし worker は I-39(proposal_id None) でクラッシュ＋I-38(再配信ストーム) を誘発＝1トリガーで18冊。themeKind=serendipity の distinct-theme 結果はストーム停止のため purge＝未観測。(4) バックエンドゲートは I-41(ALLOWED_TRIGGER_UIDS空) で未防御。⇒ I-38/I-39 は✅修正済・I-41も✅解決済。**【2026-06-29 MTG＝デモは"事前準備した本の入荷表示"に特化】** デモではこのボタンを**ライブ生成には使わない**＝押下で裏にあらかじめ用意した本が書庫/書店へ"入荷"される見せ方にする（**実LLMを呼ばない＝課金ゼロ・安全**）。見せ場は「入荷の様子」と「読書ページ」。無認証公開デモ（I-32方針転換）の安全性の担保もこれ。**この入荷見せ方（事前準備本表示）の実装担当＝一瀬に委譲（2026-06-29 デモ環境タスク移管）。****【MTG 2026-06-18・3.3・G1-6・I-38/I-39/I-41・MTG 2026-06-29】 |
| I-32 | **デモ環境ID/Password認証** | 🟢**方針転換でクローズ（2026-06-29 MTG）＝認証は設けない** | ~~MTG 2026-06-18：デモ環境への認証として ID/Password 方式を実装（demo-token＋ログインフォーム・commit 72c0eb1）~~ → **【2026-06-29 MTG で方針転換＝ID/Password 認証は不要】**。デモは**無認証の公開デモ**に確定＝審査員含め誰でも、**佐倉美咲アカウントのページを既定表示**で閲覧できる状態にする（**パスワードは設定しない**）。安全性は「**ライブ生成させない**＝事前準備した本を"入荷表示"するだけで実LLM/課金を発生させない」ことで担保（I-31参照）。実装済みの demo-token/ログインフォーム（72c0eb1）はこの方針では未使用（**撤去要否は一瀬のデモ調整で判断＝2026-06-29 デモ環境タスクを鉄田→一瀬へ委譲**）【MTG 2026-06-29・I-31】 |
| I-33 | **著者お気に入り（favoriteAuthors）状態保持バグ（優先度: 高）** | ✅**本番実機検証済（2026-06-24）**：登録/解除とも reload 跨ぎで Firestore 永続・既存4件も初期ロード | `favoriteAuthors` を Firestore `users/{uid}.favoriteAuthors` から購読して `useFavorites()` に反映。削除は `personaId` で既存配列を探し、Firestore上の実オブジェクトを `arrayRemove` する。保存エントリには schema 必須の `voiceStyle` / `format` を補完し、読了ページのお気に入りボタンも実保存へ接続【MTG 2026-06-18・WBS C4.6・favorites-store.ts・user-writes.ts】 |
| I-34 | **GitHub公開用新規パブリックリポジトリ作成（PII除去・履歴クリーン）** | 🔜着手前・**担当=一瀬（2026-06-29 MTG）** | **【2026-06-29 MTG】作業担当を一瀬に確定**（鉄田は関与せず委譲）。**MTG 2026-06-18 で方針確定**。既存リポ（`cloud-dojo/publishr`）には PII（個人を特定できる情報）や内部ドキュメントの履歴が蓄積されており、完全なスクラブは技術的整合性の観点から困難と判断。方針: 現行リポを**「開発・プライベート用」として隔離保持**し、外部公開・本番用として**履歴クリーンな新規パブリックリポジトリを別途作成**する（コードと docs/design のみコピー・計画系 docs は除外）。WBS C6.6/C6.5 の公開クリーンリポ作業と統合【MTG 2026-06-18・3.4・WBS C6.6】 |
| I-35 | **本文生成ボリュームのパラメータ化（3000文字以上への拡張）** | ✅**本番実機検証済（2026-06-24）**：mock12件＋prod実Vertex歩留まり実測（per-chapter 1500目標に対し実~2400字/章＝~1.6倍overshoot・prod目標は要調整） | **MTG 2026-06-18 で確定→2026-06-24 実装**。「本全体」の目標文字数 `body_char_target` を制御ノブに採用＝`config.py`(BFF・既定12,000字)＋実行プロファイル(`runtime.py` dev1,500/prod12,000・`PUBLISHR_BODY_CHAR_TARGET` 上書き)に追加。`mode_b/vertex_agent._resolve_volume()` が profile/env から本全体目安を解決し、**死んでいた system プロンプト `{{body_volume}}` を生かして注入**＋各章＝本全体÷採用章数で導出（章単位の明示指定 `PUBLISHR_BODY_CHARS_PER_CHAPTER`＝CLI `run_full_book --chars` 互換は最優先）。mock(決定的)経路は不変＝ゼロ差分。test_mode_b/test_runtime_guard/test_config に既定・env上書き・導出のテスト追加。**残＝実Vertexで実文字数の歩留まり実測→prod目標値の最終調整**【MTG 2026-06-18・3.1・apps/api/publishr_api/config.py・agents/publishr_agents/mode_b/vertex_agent.py】 |
...
| I-38 | **🔴緊急: Pub/Sub 再配信ストーム（1トリガー→18冊の重複本）** | ✅**修正＋本番ライブ検証済（2026-06-24）** ＝安定runId＋Firestore冪等ロック（`planningJobs/{runId}`）＋runId由来の決定的book ID。**ライブ実証**: 本命1トリガー→600sで Pub/Sub 再配信が発生したが worker が冪等ロックで `skip+ack`→**入荷は4冊で打ち止め（旧:18冊）・storm消滅**。`planningJob=completed`。併せて**遅いrunのreap対策で Cloud Run request timeout を 600s→3600s に引き上げ**（rev publishr-api-00097）＝初回runが完走してpersistまで到達（push subの ack_deadline は600s上限のため再配信は起きるがロックで無害化）。verify 416 passed | **本番実機QAで「今すぐ企画」本命を1回叩いたら 18冊が生成された**。原因連鎖: ①`publishr-planning-push`/`publishr-writing-push` の **ackDeadline=600s** に対し本命run(4冊×Imagen表紙+I-35本文+I-39クラッシュ)が**600秒超過**→worker が ack 前に Pub/Sub が**再配信**→約10分おきに再実行（17:42/17:52/18:02/18:13/18:23 の5世代×3-4冊） ②**dead-letter なし(maxDeliveryAttempts 無制限)**＝自動で止まらない ③**冪等性なし**＝再配信ごとに run-uniqueID が変わり新IDの別本になる（dedupされない） ④I-39クラッシュで正常ackできず悪化。**暫定対応**＝両サブスクを `gcloud pubsub subscriptions seek --time=now` で purge して再配信停止＋ストーム本18冊を `cleanup_arr_books.py --apply` で削除。**根本対処（手動/自律トリガーを安全にする前提）**＝ack期限内に収める非同期分割 or ack延長 or 冪等キー(messageId/bookId)でのスキップ or dead-letter上限。prod-live-followups の「再配信は稀＝MVP許容」は**実態と乖離**（稀でなくストーム）【worker.py・services/write_queue.py・prod-live-followups #1】 |
| I-39 | **本命/セレンディピティ worker が PipelineResult ValidationError でクラッシュ（cast_None）** | ✅**修正＋本番ライブ検証済（2026-06-24）** ＝`vertex_set._run_team` が proposalId 欠落時に `plan_vertex_{team_id}` を必ず付与＋`mode_a_service` で None filter。ライブ本命runが `planningJob=completed`（4 bookIds・cast_None/ValidationError 無し）で完走。test_mode_a_set/test_planning_set_vertex に非None/一意 assert | set pipeline(vertex)で `mb.plan.proposal_id` が None になり、`mode_a_service.py:202` の `PipelineResult(approved_plan_ids=[...])` が `string_type` 検証で落ちる（books は persist 後にクラッシュ＝本は残るが pipeline は失敗ack→I-38再配信を誘発）。RCA＝`planning/vertex_set.py` が LLM 出力に proposalId 欠落時に proposal_id を付与しない（deterministic.py は team_id で付与する）。schema `agent_io.py` の `proposal_id: Optional[str]=None` が許容。persona_id も `cast_{None}`＝`cast_None` に。**最小修正**＝vertex_set で approved 各 plan に proposal_id を必ず付与＋`mode_a_service` 側で None を filter。導入＝PR#83(4テーマ移行)【vertex_set.py・mode_a.py:128・mode_a_service.py:205】 |
| I-40 | **plans onSnapshot が全件queryで permission-denied** | ✅解決済み（PR#92・2026-06-26、本タスクで正本更新2026-07-01） | `firestore-provider.ts` の `plans` 購読が **ownerUid filter 無し**で `onSnapshot(collection("plans"))` していた→rules `allow read: ownerUid==uid` がコレクションqueryを拒否→`permission-denied`（コンソールに恒常エラー）。PR#92（1567a9c）で `where("ownerUid","==",uid)` を付与済み（books listener と同様）。正本の状態記載が実態に追いついていなかったため本タスクで同期【firestore-provider.ts:104】 |
| I-41 | **BFF `ALLOWED_TRIGGER_UIDS` 空＝バックエンド・トリガーゲートが開放** | ✅**解決済み（2026-06-25・一瀬対応）** | 一瀬が Cloud Run env に `ALLOWED_TRIGGER_UIDS=5JLLGOc3rpXiGN9KXmsISBNAKty2` を設定済み。フロント(`NEXT_PUBLIC_TRIGGER_UIDS`)＋バックエンド(`TriggerGuard`)の両層が佐倉のみ許可で揃った【config.py:87・trigger_guard.py・prod-live-followups #7】 |
| I-42 | **境界ケース(eval_b1/b2)の設計が実judgeの挙動と乖離** | 🟡新規（2026-07-01発見・軽微） | C5.4実judge再現性測定（`eval_reproducibility --backend vertex` n=5）で判明。eval_b1（"ギリ通す"狙い・band[68,80]）は5回とも83付近で安定して**上振れ**（σ0.63・band内0/5）。eval_b2（"ギリ落とす"狙い・band[50,68]）は5回とも29付近で安定して**下振れ**（σ2.53・band内0/5、低関連2件と同水準）。**「ブレて不安定」ではなく「一貫してband外」**＝mock judge（キーワードマッチ）向けに設計されたband値が、実judge（意味理解）の閾値70近傍の真の刃先を捉えられていない。C5.3ゲート（8件）合否には無影響（borderlineは診断専用・ゲート計算対象外）。対処候補: (a) band値を実データ(b1≈83/b2≈29)に合わせて再定義、(b) 真に70近傍で揺れる新しい境界企画をeval_set.yamlに作成、(c) 現状のまま「参考値・非拘束」と明記するに留める。優先度は低い（提出7/10までの必須ゲートではない）【scripts/eval_reproducibility.py・eval/eval_set.yaml borderlineCases・I-1・2026-07-01】 |
...
| P-7 | **体験画像は実フロントのスクショ（佐倉/7名）で撮る／図のクリーン公開版** | 🟡**撮影待ち（UI Fix済）** | UIモック（`docs/ui/mockups`）は"30人"前提で佐倉(7名)と矛盾＝**最終画像は実フロントのスクショに差替前提**（モック自体は直さない）。**2026-06-30: UIはPR#95でFix済み**（main merge `40840c8`・主要ルートHTTP smoke 200）なので、体験画像①②③はFix済み実フロントから撮影する段階。図④⑤・システム構成図は内部値（閾値70/週15冊/100p/内部コード名）を含むため**公開版はマスク要**（保留中）。**公開クリーンリポ**（コード＋設計のみ・計画系除外）も提出までに用意【画像プラン.md・README 公開時運用・WBS C6.8】 |
...
2. ~~**W1：ADK最小マルチエージェント疎通**（W1技術論点・最大の技術リスク）~~ ✅**完了（2026-06-06・W0前倒し）**＝C1.0.1 MiniLoopが実Vertexでescalate脱出を実証（wbs.md M1達成）
3. ~~**W2：E2E縦通し1本**（プロジェクトの主要マイルストーン・ARCH §11 R3）~~ ✅**完了（2026-06-09・前倒し）**＝M2達成（PR#4・Cloud Run/Firestore縦通し・実モードAトリガー）
4. **デモ台本でスコア閾値の差し戻しカットを設計**（P-5／基準1を映す）🟡状態未確認＝本台帳のP-5は「方針OK・未実証」のまま（審査での見せ方§参照）。動画台本自体(C6.1)は完成済みだが、本項目（差し戻しカットの具体設計）が反映済みかは要確認
5. ~~Eval Set 8件＋サンプルDrive＋initialProfile選択肢（G1-9/G1-10・鉄田）~~ ✅**完了**（2026-06-04 initialProfile確定でクローズ）

> **【2026-07-01 plan-audit棚卸し】** 上記TOP5は2026-06-05時点の優先順位で作られたもので、機能凍結(7/2)・提出(7/10)が目前の現在は実質的な役割を終えている。項目2・3クローズにより残る未決は項目4のみ。以後の最優先論点はwbs.md「現在地サマリ（2026-06-30）」が示す**C6（デモ録画/ProtoPedia提出物・M5/M6）**に移行済み。

---

## 決着済みログ（参考・蒸し返さない）

- ✅ **【定例MTG・2026-06-29 決定】**：①**機能凍結 6/30→7/2 へ延長**（表紙・obs4 の品質詰めの猶予／提出 7/10 据置）②**デモ＝無認証の公開デモに確定**（審査員含め誰でも**佐倉アカウント既定表示**で閲覧・**パスワードなし**＝I-32 方針転換でクローズ）＋**「今すぐ入荷」ボタンは事前準備した本を"入荷表示"する見せ方に特化＝ライブ生成しない・課金ゼロ・安全**（I-31）＋**このデモ環境系タスク（無認証公開デモ・佐倉既定表示・I-31入荷ボタン挙動・I-32認証撤去）の実装担当は一瀬に委譲（2026-06-29）**③**表紙（C5.1）＝6/30 まで実Imagen表紙を鉄田が継続挑戦、間に合わなければ簡易装丁（既定・`0f48ae5`）で確定**④**企画会議の証跡（却下→再提出）はUIに見せ場を置かず Langfuse で追える状態で十分**（合意）⑤**日曜セレンディピティ運用＝最終確認OK・完了**⑥**公開クリーンリポ（I-34）＝一瀬に委譲**⑦継続担当＝**プロンプトのテイストチューニング＋UI整理＝鉄田**（UI整理は2026-06-30にPR#95でFix済み）【MTG 2026-06-29】
- ✅ **【UI Fix完了・2026-06-30】鉄田担当のUI最終調整はPR#95でmainマージ済み**：書店/本棚の用語統一、トップ・本棚・ハイライト/ブックマーク・作家・サイトマップ・アカウント・本概要・読書・読了ページの密度/余白/見出し/表紙/文言調整、旧仕様文言（予約/週次/入荷強調/セレンディピティタグ等）除去、ブラウザ戻る導線を修正。CI `verify` 成功、main取り込み後 `localhost:3000` の主要ルートHTTP smoke 200。以後UIはFixとして、P-7/C6.7の実フロントスクショ撮影へ進む【PR#95・merge `40840c8`・WBS C4.8/C6.7】。
