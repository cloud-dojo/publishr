...
| I-23 | **lint（make verify の lint-web）が赤** | 🟡要整理（buildは通る） | `react-hooks/set-state-in-effect`（`app/(shell)/page.tsx`初回体験・`account/page.tsx`連携読込／一部は意図的に`eslint-disable`済）＋`components/AuthSync.tsx` の `@typescript-eslint/no-explicit-any`（既存）。**App Hostingの`next build`はeslintをデプロイブロックしない設定のため本番デプロイは成功**（本セッションの全push成功実績）が、CLAUDE.md「make verify 緑」と齟齬。AuthSyncのany解消＋effect内setStateの整理で緑化可能【2026-06-07】 |
