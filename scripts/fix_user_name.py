...
seed で別ユーザーの fixture を上書きした名残（例: 別メンバーuidの name が「佐倉 美咲」）や
オンボーディングのみで name 未設定のアカウントを、本人名へ直す。
name/initial 以外のフィールドは updateMask で温存する。

使い方:
  python scripts/fix_user_name.py --uid <UID> --name "佐倉 美咲" --initial 佐            # ドライラン
  python scripts/fix_user_name.py --uid <UID> --name "佐倉 美咲" --initial 佐 --apply    # 実更新
"""
