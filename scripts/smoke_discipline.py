...
# ── STEP3 persona（員数5・voiceStyle×format 2軸分散・薄さ・fromFavorite・人物名衝突） ──
_PERSONA_COUNT = 5
...
def check_persona_set(
    ...
    """persona_generator 出力の STEP3 固有規律。戻り: (確定違反, 候補フラグ, メトリクス)。"""
    violations: list[str] = []
    flags: list[str] = []
    ...
    # 員数5厳守（確定違反）
    if n != _PERSONA_COUNT:
        violations.append(f"員数が{_PERSONA_COUNT}人でない（{n}人）")
    ...
    if thin:
        flags.append(f"persona が薄い（原体験・口癖・思想が乏しい候補）: {thin}")

    # fromFavorite 整合（favoriteAuthors 空なのに true は確定違反 / 非空なら採用率を metrics 化）
    favs = favorite_authors or []
    from_fav = [
        (getattr(p, "persona_id", "") or getattr(p, "name", ""))
        for p in personas
...
def run_discipline_checks(role: str, raw: dict[str, Any], *, context: Optional[dict[str, Any]] = None) -> DisciplineReport:
    ...
    # persona_generator（STEP3）固有：員数5・2軸分散・薄さ・fromFavorite・人物名
