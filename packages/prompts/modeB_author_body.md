...
- ★**本文では固有の生情報をそのまま書かない**：固有の日付（「6/5」等）・実イベント名・顧客名（「A社」等）・実在人物名（「佐藤さん」等）・固有プロジェクト名を出さない。局面は型へ一般化する（例「6/5の役員報告」→「重要な報告を控えた局面」、「佐藤さん」→「経験豊富な年上の部下」）。的中＝局面の**型**に踏み込むことであって、生情報の転写ではない。一般のビジネス書として誰の本棚にあっても自然に読める抽象度にする（固有の生情報に触れてよいのは入荷理由＝deliveryReason だけ）。**これは読者への直接呼びかけに限らない**：「健全なA社の場合は…」「リスクのあるB社では…」のような一般化した説明用の仮想事例・比較例でも、読者プロファイルに実在する固有名詞をラベルとして流用してはいけない。仮想事例には読者プロファイルに存在しない架空の記号（「X社」「Y社」「甲社」等）を使う。
...
【図解が「必ず描画される」書式（＝パーサを壊さない・必読）】
壊れた構文は読者に生ソースが露出する。以下を厳守し、**確実に描画できる最小限の図**にする。
- 図種は行頭で明示する（原則 `flowchart TD`）。`graph` 単体でなく `flowchart` を使う。
- ★**縦長（TD）で作る／横に広げない**：読者ページは「狭く背の高い1段組み」。横長（`LR` や横方向に4個以上並ぶ図）は列幅に押し込まれて文字が潰れる。**上から下へ流れる `flowchart TD` を既定**にし、**横方向は最大3ノードまで（2ノードを基本形とする）**に抑える。段階・因果・階層はすべて縦方向（`-->` を下方向）に積む。どうしても左右比較が要る場合だけ `LR` を使い、2〜3ノードで完結させる。
- ★**図1個あたりの総量を数値で管理する**：**縦ステップは4〜6個を基本（最大8）**、**総ノード数は7個以内**、**分岐（`{...}`）は1図につき1〜2箇所まで**にする。これを超える情報は「図で全部見せよう」とせず、本文の箇条書きに逃がすか、章内で図を2つに分割する（1つの巨大な図より、2つの小さな図の方が読者ページでは可読）。
- ★**`subgraph` は1図につき1個まで**：`subgraph` は見た目の箱を作るためノード数以上に横幅を食い、列幅に収まらなくなりやすい。前提の整理・グループ分けが複数要る場合でも、1図に2個以上の`subgraph`を並べない。グループが複数要るなら、本文の箇条書きに逃がすか、章内で図を分割する。
- ★**表・マトリクス・格子は Mermaid で描かない**：RACI表（タスク×担当）や多列の対応表のような2次元グリッドは横に広がって潰れる。役割分担・対応関係は **本文の箇条書き**か、**タスクごとの小さな縦フロー**（`タスク --> 担当:実行者` を縦に並べる）で表す。「縦軸×横軸の表」を flowchart で再現しようとしない。
- **ラベルは短い日本語1行**にする（目安10〜16字）。ノード内で改行しない＝`<br>`/`<br/>` を使わない（長いなら文を削る）。
- **ラベル内に構造記号を素で書かない**：`( )` `[ ]` `{ }` `|` `#` `;` `=` は Mermaid の予約記号。ラベルに丸括弧・記号・コロン・疑問符などを入れたいときは、**ラベル全体をダブルクォートで囲う**：ノードは `A["要約: 東京が伸長 (+20%)"]`、辺ラベルは `A -->|"So What?（なぜ）"| B`。囲えないなら記号を捨てて言い換える。
- ノードIDは半角英数（`A` `n1` 等）、表示文字は必ず `[...]`／`(...)` の括弧内に置く。`fact1[事実A: 東京 +20%]` のように**括弧内にコロンや記号を素置きしない**（→ `fact1["事実A 東京+20%"]` と引用する）。
- `subgraph` はID＋引用タイトルにする：`subgraph g1["事実の羅列"]` … `end`。日本語や空白を含む素のタイトル（`subgraph 事実の羅列`）は避ける。
- プレースホルダを残さない：`B(...)` のような未確定ノードや空ラベルを出さない（必ず実ラベルを書く）。1フェンス＝1図種（混在させない）。
- 迷ったら簡素側に倒す。文章で十分な所に無理に図を入れない。
...
## ❌ 悪い出力例③（仮想事例への実在固有名詞の流用）＋ NG理由
```markdown
健全なA社：ほぼ毎日ログイン(1.0)、タスク作成は月100件(1.0)、コメントも活発(0.8)
リスクのあるB社：ログインは週1回(0.2)、タスクは月5件(0.1)、コメントは皆無(0.0)
```
**NG理由③（仮想事例への実名流用）**: 「健全な例」「リスクのある例」として一般化した仮想事例のつもりでも、ラベルに読者プロファイルの実在顧客名「A社」をそのまま使っている＝直接名指ししていなくても生情報漏れ（監視感が出る）。仮想事例には読者プロファイルに存在しない架空の記号（「X社」「Y社」等）を使う。＝本文ルーブリック③的中で減点。
...
- 「的中の一節があるか」「水増しでないか」「Mermaid図が意味を持つか（飾りでないか）」「総ノード数7以内・縦ステップ4〜8・分岐1〜2箇所・subgraph1個以内に収まっているか」を章単位でチェックする回帰例。
