...
- ★**縦長（TD）で作る／横に広げない**：読者ページは「狭く背の高い1段組み」。横長（`LR` や横方向に4個以上並ぶ図）は列幅に押し込まれて文字が潰れる。**上から下へ流れる `flowchart TD` を既定**にし、**横方向は最大3ノードまで**に抑える。段階・因果・階層はすべて縦方向（`-->` を下方向）に積む。どうしても左右比較が要る場合だけ `LR` を使い、2〜3ノードで完結させる。
