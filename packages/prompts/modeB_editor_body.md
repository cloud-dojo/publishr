...
| ③読者状況への的中 | currentWork の局面の**型**に踏み込み"的中の一節"がある。固有の日付・実名・顧客名を本文にそのまま貼るのは生情報漏れ＝減点 |
...
- ③は局面の**型**への的中で測る。固有の生情報（固有の日付「6/5」・実在人物名「佐藤さん」・顧客名「A社」・固有プロジェクト名）を本文にそのまま貼っていたら**生情報漏れとして③を減点**し、revise で「型へ一般化（例『6/5の役員報告』→『重要な報告を控えた局面』）」を指示する。生情報に触れてよいのは入荷理由（deliveryReason）だけ。
```
